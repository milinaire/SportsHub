...
class Category extends Component {
  ...
  componentDidMount() {
    this.props.cancelBox()
    this.props.setButtonElem(
      <Link to={`/admin/${this.props.match.params.category}/new_article`}>
        <div style={{background: '#d72130', width:'150px', height:'40px', display:'flex', justifyContent: 'center', alignItems: 'center', paddingBottom:'2px'}}>
          + NewArticle
        </div>
      </Link>
    )
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    if (prevProps.match.params.category !== this.props.match.params.category) {
      ...
      this.props.setCurrentSection(this.state.Categories.find(el => String(el.id) === this.props.match.params.category).name)
      fetch(`conference?languageId=1&categoryId=${this.props.match.params.category}`)
        .then(res => res.json())
        .then(
          (result) => {
  ...
  render() {
    return (
      <Fragment>
        ...
        <div>
          <AdminArticlesList category={this.props.match.params.category} conference={this.state.SelectedConference.value} team={this.state.SelectedTeam.value} published={this.state.SelectedPublishedOption.value}/>
        </div>
      </Fragment>
    );
  }
